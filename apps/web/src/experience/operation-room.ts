...
const createRoomShell = (scene: THREE.Scene) => {
  ...
  const doorMinZ = -2.95 - ROOM_DOOR.width / 2
  const doorMaxZ = -2.95 + ROOM_DOOR.width / 2
  ...
  // Right-wall brown door. It uses the exact same shared geometry as the
...
const createBackCloset = (scene: THREE.Scene) => {
  ...
  // The black doorway uses the exact same casing geometry as the brown door.
  ...
  // The animated black leaf is the same shared door model as the brown one.
...
const createFilmReelStorage = (scene: THREE.Scene) => {
  ...
  // Three cased reels remain in the box; two matching reels are stored loose
